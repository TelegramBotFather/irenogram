...
def pyrogram_api():
    ...
    with open(root + "/index.rst", "w", encoding="utf-8") as f:
        ...
        for subdir, (display, methods) in method_categories.items():
            ...
            for _fname, actual in methods:
                if actual in STANDALONE_METHODS:
                    lines.append("    * :func:`{} <pyrogram.{}>`".format(actual, actual))
                else:
                    lines.append("    * :meth:`{} <pyrogram.Client.{}>`".format(actual, actual))
            lines.append("")
            lines += [".. toctree::", "    :hidden:", ""]
            lines.append(
                "    " + "\n    ".join("{0} <{0}>".format(actual) for _fname, actual in methods)
            )
            lines.append("")
    ...
    with open(root + "/index.rst", "w", encoding="utf-8") as f:
        ...
        for key, (display, types_list) in type_categories.items():
            markup = "-" * len(display)
            lines += [display, markup, ""]
            lines += [".. autosummary::", "    :nosignatures:", ""]
            lines.append("    " + "\n    ".join(types_list))
            lines += ["", ".. toctree::", "    :hidden:", ""]
            lines.append("    " + "\n    ".join(types_list))
            lines.append("")
